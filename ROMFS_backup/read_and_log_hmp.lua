...
function decode_and_write_to_flash(fullmessage)
  ...
  if count==3 then
	...
	logger.write('HMP','Temperature,Humidity,Dewpoint','fff',table.unpack(log_data))
	msg_count = msg_count+1
    if msg_count == nth_msg2display then
      gcs:send_text(5, string.format("HMP %.1f degC @ %.0f pcRH",log_data[1],log_data[2]))
	  gcs:send_named_float('Temperature (°C)',log_data[1])
	  gcs:send_named_float('Humidity (% rF)',log_data[2])
	  msg_count = 0
    end
  else
