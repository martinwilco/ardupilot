...
#define SDP3Xmult_SCALE_TEMPERATURE		200.0f

#define SDP3XmultD0_I2C_ADDR            	0x00
#define SDP3XmultD1_I2C_ADDR            	0x00
#define SDP3XmultD2_I2C_ADDR            	0x23

#define SDP3Xmult_CONT_MEAS_AVG_MODE    	0x3615
#define SDP3Xmult_CONT_MEAS_STOP        	0x3FF9

#define SDP3Xmult_SCALE_PRESSURE_SDP31	60
#define SDP3Xmult_SCALE_PRESSURE_SDP32	240
#define SDP3Xmult_SCALE_PRESSURE_SDP33	20

extern const AP_HAL::HAL &hal;

AP_Airspeed_SDP3Xmult::AP_Airspeed_SDP3Xmult(AP_Airspeed &_frontend, uint8_t _instance) :
 ...
 */
bool AP_Airspeed_SDP3Xmult::_send_command(uint16_t cmd)
{
...
bool AP_Airspeed_SDP3Xmult::init()
{
    const uint8_t addresses[3] = { SDP3XmultD0_I2C_ADDR,
                                   SDP3XmultD1_I2C_ADDR,
                                   SDP3XmultD2_I2C_ADDR
                                 };
    bool found = false;
    bool ret = false;
    ...
    for (uint8_t i=0; i<ARRAY_SIZE(addresses) && !found; i++) {
        ...
        if (!_send_command(SDP3Xmult_CONT_MEAS_STOP)) {
        ...
        if (!_send_command(SDP3Xmult_CONT_MEAS_AVG_MODE)) {
            _dev->get_semaphore()->give();
            continue;
        }
        ...
        char c = 'X';
        switch (_scale) {
        case SDP3Xmult_SCALE_PRESSURE_SDP31:
            c = '1';
            break;
        case SDP3Xmult_SCALE_PRESSURE_SDP32:
            c = '2';
            break;
        case SDP3Xmult_SCALE_PRESSURE_SDP33:
            c = '3';
            break;
        }
    ...
    _dev->set_device_type(uint8_t(DevType::SDP3Xmult));
    ...
    _dev->register_periodic_callback(20000,
                                     FUNCTOR_BIND_MEMBER(&AP_Airspeed_SDP3Xmult::_timer, void));
    return true;
}

// read the values from the sensor. Called at 50Hz
void AP_Airspeed_SDP3Xmult::_timer()
{
    ...
    if (!ret) {
        if (now - _last_sample_time_ms > 200) {
            // try and re-connect
            _send_command(SDP3Xmult_CONT_MEAS_AVG_MODE);
        }
        return;
    }
    // Check the CRC
    ...
    float temperature = float(temp) / SDP3Xmult_SCALE_TEMPERATURE;
 ...
 */
float AP_Airspeed_SDP3Xmult::_correct_pressure(float press)
{
    float sign = 1.0f;
...
bool AP_Airspeed_SDP3Xmult::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AP_Airspeed_SDP3Xmult::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
 ...
 */
bool AP_Airspeed_SDP3Xmult::_crc(const uint8_t data[], uint8_t size, uint8_t checksum)
{
